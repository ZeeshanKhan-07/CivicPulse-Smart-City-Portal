...
function RegisterComplains() {
    ...
    const handleSubmit = async (e) => {
        ...
        await toast.promise(
            submissionPromise,
            {
                ...
                error: (err) => `Error: ${err.message || 'Failed to connect to server.'}`,
            }
        );
    };
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gray-100 py-10 px-4">
            ...
            <div ref={containerRef} className="max-w-4xl w-full p-10 bg-white shadow-2xl rounded-2xl border-t-8 border-indigo-600">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 border-b border-gray-200 pb-4">
                    <div className="flex flex-col mb-4 md:mb-0">
                        ...
                        <p className="text-md text-gray-500 mt-1">
                            Lodge a complaint to the municipal corporation.
                        </p>
                    </div>
                    <div className="flex flex-col items-start md:items-end space-y-2 pt-1">
                        <p className="text-sm text-gray-600">
                            Logged in as: <span className="font-semibold text-indigo-700">{userEmail}</span>
                        </p>
                        <div className="flex space-x-3">
                            <Link
                                to="/status"
                                className="px-4 py-2 text-sm font-medium text-indigo-600 border border-indigo-600 rounded-xl hover:bg-indigo-50 transition-colors duration-200"
                            >
                                📈 Track Status
                            </Link>
                            <button
                                onClick={handleLogout}
                                className="px-4 py-2 text-sm font-medium bg-gray-200 text-gray-700 rounded-xl hover:bg-gray-300 transition-colors duration-200"
                            >
                                Logout
                            </button>
                        </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        {/* City Dropdown */}
                        <div ref={addAnimatableRef}>
                            <span className="text-gray-700 font-bold text-sm mb-1 block">City (Mandatory)</span>
                            <select
                                name="city"
                                value={form.city}
                                onChange={handleChange}
                                className={`mt-1 block w-full p-3 border border-gray-300 rounded-xl focus:ring-indigo-500 focus:border-indigo-500 bg-white cursor-pointer appearance-none pr-10 transition-all duration-200 ${!form.city ? 'text-gray-400' : 'text-black'}`}
                                required
                            >
